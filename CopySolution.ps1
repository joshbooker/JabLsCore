...
if ( (Test-Path $sourceSolutionFolder) -and ((Get-ChildItem -Path $sourceSolutionFolder -filter *.sln).Count -eq 1) -and ((Get-ChildItem -Path $sourceSolutionFolder -Recurse -include *.lsxtproj, *.csproj, *.jsproj).Count -gt 2)) {
    ...
    try {
		...
		# =================================================================
        # Create variables for where JabLsCore is and will be
        # =================================================================
		$JabLsCoreLocation = (Get-ChildItem -Recurse -Filter "*JabLsCore-*" | select -First 1 directory)
		if ($JabLsCoreLocation -ne $null) {
			$oldLsWiresPath = "*\" + $JabLsCoreLocation.Directory.Parent.Name + "\" + $JabLsCoreLocation.Directory.BaseName
			$newLsWiresPath = "*\" + "$newSolutionName.$newClientName" + "\" + $JabLsCoreLocation.Directory.BaseName
		}
		...
		# ITG Fix - Added Exclude for the JabLsCore folder
		if ($JabLsCoreLocation -ne $null) {
